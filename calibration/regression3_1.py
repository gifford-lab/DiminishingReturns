#Continues running from previous script
...
class regressor(nn.Module):
    def __init__(self, data, windowSize, initial):
        ...
        self.dx = nn.Parameter(torch.tensor(initial/normalize, dtype = dt), requires_grad=True)
...
def fitmodel(data, window, iterations, init, lr):
    model = regressor(data, window, init)
    model = model.cuda()
...
r1a = fitmodel(mhc1a, 1000, 100000, it1a, 5)
#r2a = fitmodel(mhc2a, 1000, 100000, it2a, 10)

with open("./calicurve32_200k_1a.pkl", 'wb') as fout:
    pickle.dump(r1a, fout)